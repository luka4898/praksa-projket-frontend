...
const Home = (props) => {
  ...
  useEffect(() => {
    (async () => {
      let res = await fetch(
        `https://localhost:7100/api/CurrentEvents/getallcurrentevents?$filter=numberOfSeats lt 10 and end gt ${moment().toISOString()}`,
        {
          credentials: "include",
        }
      );
      ...
      {similarevn && (
        <>
          ...
          <div className="container">
            ...
            <MultipleCarousel similarevn={similarevn} history={props.history} />
          </div>
        </>
      )}
    </>
